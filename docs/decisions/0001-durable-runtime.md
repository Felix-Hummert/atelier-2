...
`fe7bd6418da17f05ff8a1b09e341042d98fec8a5c54237a7ebf1311be472c2ae`.
...
| V26 redemption beside the node receipt | A fresh exact V26 store retargets `tool_redemptions.node_execution_id` from `agent_receipts_v2` to `node_receipts_v3`, so a granted verification that exits nonzero can keep the command, exit code and output hash beside the `failed` node receipt that attempt actually has. A populated exact V25 store migrates through one transactional `tool_redemptions` rebuild that keeps every stored row; predecessor rows already sat beside a succeeded V3 node receipt and nothing is reinterpreted; V25 joins the refused predecessors at runtime. |
