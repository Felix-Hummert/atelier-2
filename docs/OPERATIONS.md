...
and rebuilds the commit reported by the running Serve, restarts it, and still
exits nonzero. A `live serve is DOWN, operator action needed` line means that
recovery itself failed; inspect `journalctl --user -u atelier2-serve.service -e`
before acting.
