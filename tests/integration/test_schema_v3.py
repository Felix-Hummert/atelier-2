...
def test_fresh_v11_has_the_closed_product_tables_and_reopens_idempotently(
    tmp_path: Path,
) -> None:
...
def test_existing_v11_rejects_every_product_schema_drift_without_mutation(
    ...
    with pytest.raises(UnsupportedSchemaVersion, match="malformed v11"):
...
def test_existing_malformed_in_memory_v11_is_refused() -> None:
...
def test_dbos_owned_tables_are_allowed_and_unchanged_by_v11_preflight(
    tmp_path: Path,
) -> None:
    database = tmp_path / "atelier.sqlite"
    engine = create_canonical_engine(database)
