...
`workflows/schemas/`; the house work-item schema stays with its production
contract. A workflow whose pin nobody could ever publish is exactly as
unstartable as one that fails to parse, so this suite treats both as the same
class of defect.
