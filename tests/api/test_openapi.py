...
EXPECTED_ROUTE_SEQUENCE = (
    ...
    ("POST", API_PREFIX + "/schema-revisions", "publish_schema_revision"),
...
def test_served_document_is_byte_identical_to_the_frozen_artefact() -> None:
    ...
    The artefact was last regenerated when `POST /schema-revisions` opened the
    catalog publish path for schema bytes: hash out, created-versus-existing,
    and a named refusal for a document the profile will not enforce. That
