"""The #301-A witness's hook into the host's own owners: a thin command line.

The inspect attestation itself is `atelier2.adapters.docker_carrier` and the
identity material's own reader is `atelier2.host.runner_identity`; this script
only gives the witness's shell a way to reach them, plus the one manifest a
real Serve would publish for the generation it bound. Issuing identities is no
longer among them: `atelier2-runner-launcher issue-console-identity` is the
production command for the console's, and the launcher itself mints every
Runner's. No key material and no decision lives here, and nothing here is ever
...
from atelier2.host.runner_identity import receiver_record, unlink_private_keys
