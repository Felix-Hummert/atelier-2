...
// carry the same `true` this fixture now states explicitly. `provider_probe_
// problem_code` and `provider_probe_observed_at` added by hand the same way
// (#1103): both configurations already carried a valid receipt, so a
// re-capture today would carry the same `null` pair this fixture now states
// explicitly.
