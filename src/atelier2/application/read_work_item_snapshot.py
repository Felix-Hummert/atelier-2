...
translates the same source's words -- and the two words about the connection
itself, an unconnected instance and a payload its adapter refused, stay that
module's rather than becoming a second copy under this one.
"""
...
def read_work_item_snapshot(
    ...
    item_reference = WorkItemReference(project, reference)
    match source.snapshot(reference):
        case WorkItemRevisionObserved(revision):
            return WorkItemSnapshotRead(item_reference, revision)
        case TrackerItemUnknown():
            return WorkItemNotInTracker(item_reference)
